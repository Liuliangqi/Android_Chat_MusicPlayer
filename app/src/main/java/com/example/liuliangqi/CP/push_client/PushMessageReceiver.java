package com.example.liuliangqi.CP.push_client;
...
import com.example.liuliangqi.CP.Utils.NetUtil;
...
import com.example.liuliangqi.CP.fragments.ChatFragment;
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (intent.getAction().equals(PushConstants.ACTION_MESSAGE)) {
            ...
            for (int i = 0; i < ehList.size(); i++)
                (ehList.get(i)).onBind(method, errorCode,
                        content);
            ...
            for (int i = 0; i < ehList.size(); i++)
                (ehList.get(i)).onNotify(title, content);
        } else if (intent.getAction().equals(
...
//
        //点击跳转到的页面
        Intent intent = new Intent(application, ChatFragment.class);
        ...
        Notification notification = new Notification.Builder(mContext)
                .setContentTitle(message.getNick())
...
//                tickerText, contentIntent);
        ...
        mNotifiManager.notify(NOTIFY_ID, notification);
